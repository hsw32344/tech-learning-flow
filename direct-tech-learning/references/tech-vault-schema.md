...
For route, ledger, or schema changes, read this contract. Ordinary study follows the bounded session protocol and loads storage rules only when writing material; it does not preload this full schema. Do not add undeclared fields, headings, folders, note types, statuses, dashboards, indexes, or automations.
...
- Resolve the target Vault in this order: an explicit path the user gives; the tracked pointer `%USERPROFILE%\.agents\tech-learning-flow\vault-path.txt`; otherwise ask the user once for the Vault path.
- Never infer the target Vault from the current directory and never search unrelated directories for another Vault.
- On first use, `direct-tech-learning` or `study-tech-learning` creates the minimal valid Vault skeleton from its bundled `assets/vault-seed` by running `scripts/init-vault.ps1`; other Skills ask the user to initialize through one of them. The script creates missing directories and files, never overwrites existing content unless `-Force`, records the pointer, writes `.tech-vault.json`, then runs `scripts/validate-vault.ps1` and reports `valid`. To bind an existing Vault without copying seed files, use `-Bind`.
- The Vault owns the outermost schema: `.tech-vault.json` embeds the full structural schema plus `schema_version` and instance metadata. The bundle ships only a default seed schema (`assets/vault-schema.json`) used when creating a new Vault. On later use, read `.tech-vault.json` and re-run `scripts/validate-vault.ps1`; it loads the Vault's own schema, falls back to the bundled default with a warning when the schema is absent, verifies the Vault against that schema, and reconciles `schema_version` against the supported version. Never silently rewrite user content. By default it runs the lightweight core check (routing essentials only); run it with `-Strict` for the full structural contract. The `15-自由笔记/` and `25-资源区/外部/` areas are freeform and never validated; `25-资源区/学习快照/` stores snapshots and `97-临时/` holds scratch files.
...
| Verified source packet for one task slice, including an eligible snapshot-cache packet | `load-tech-learning-source`, conversation | never stored as learning evidence |
...
Path: `20-学习主线/`: `00-总览.md` (principles, current position, change conditions), `10-阶段/*.md` (one file per stage with its 6-column unit table), `20-来源声明.md` (all machine-parseable `primary_source`/`gap_source` lines), `来源注册表.md` (the authoritative source registry; falls back to the packaged `references/curriculum-sources.md` paradigm only when absent), and `岗位分析/` (job-oriented analysis, never machine-read). Scripts aggregate `00-总览.md`, `20-来源声明.md`, and `10-阶段/*.md`; they never read `岗位分析/`.

The mainline is the ordered task-package registry. It must contain:
...
4. each task package's canonical outcome, framework role, prerequisites, underlying causal model, bounded coverage/output, and learning-source declaration: exactly one machine-parseable `primary_source` line with source type and locator, plus at most one machine-parseable, explicitly bounded `gap_source` line. The exact line format is owned by `curriculum-sources.md`;
...
Before fetching a source, resolve the task snapshot and apply the verified snapshot-cache rules below. An eligible target slice is a previously verified source packet for study continuity and must be reused without reacquiring sufficient source content; shallow explanation is repaired under the session content rubric. An inaccessible, missing, or unsuitable exact section after cache resolution is a source-selection failure, not permission for study mode to invent or silently downgrade the course. `study-tech-learning` stops at that boundary and hands the failed locator and next ranked candidate to `direct-tech-learning`; only direction mode may persist the replacement.
...
3. `source_packet_status` is `verified`, or is `agent-fallback` for a task whose registered primary source is `AGENT-FALLBACK`;
...
Before reuse, run one lightweight Agent screen over only the target row, matching structured slice, and directly relevant source/blocker fields. Set the ephemeral source-packet field `agent_cache_check` to `passed`, `uncertain`, or `failed`: completeness asks whether the required fields and causal chain are present and coherent; correctness asks whether there is an obvious contradiction, impossible expected result, or known language/API conflict. Do not load a second source merely to perform this screen. `passed` permits cache reuse without network access; `uncertain` or `failed` requires acquisition of only the disputed or missing minimum source fragment. A short `agent_cache_notes` may identify that fragment.
...
- Each atomic note must be claimed by exactly one review bank with `review_kind: atomic-mirror` and `primary_atomic: <atomic_id>`. An atomic mirror must use `related_atomics: []`; it tests the same atomic question and is not a substitute for a broad synthesis prompt.
...
| `20-学习主线/` | Source-backed curriculum and position: `00-总览.md`, `10-阶段/`, `20-来源声明.md`, `来源注册表.md` (authoritative source registry), `岗位分析/` |
...
- Writers validate their own artifacts with the matching script: `validate-route.ps1` and `validate-terminology.ps1` (direct), `validate-daily.ps1` / `validate-weekly.ps1` / `validate-atomic.ps1` / `validate-reviewbank.ps1` / `validate-queue.ps1` (record), `validate-jobnote.ps1` (analyze), `validate-snapshot.ps1` (study snapshots), `check-draft.py` (study drafts). Each reuses the single strict contract through `vault-contract.ps1`, so they never drift from `validate-vault.ps1` core (readiness gate) and `-Strict` (full composed check).
