...
- `priority-evidence`: do not treat it as teaching material unless the user explicitly selects it for the current request.
...
By default, inspect at most two teaching pages for one packet and one additional official page when a technical fact needs verification. Stop earlier when the packet is sufficient. If the budget is insufficient, return the remaining gap instead of crawling the site. Expand only when the user requests a broader capture or the current section links to a directly required continuation.

Do not install dependencies, change browser or Windows proxy settings, or alter login state. Do not save bulk page copies. Save a normalized packet to an explicitly selected source workspace only when the user requests it.
...
A snapshot target satisfies the structural gate when the packaged validator passes, its unit and primary source match the resolver, `source_packet_status` is eligible, the target row and structured slice are complete, and no unresolved issue invalidates it. Before reuse, screen only that target material for coherent required fields and causal chain, plus obvious contradictions, impossible expected results, or known language/API conflicts. Do not fetch a second source for this screen. Return `agent_cache_check: passed` and `retrieval_mode: snapshot-cache` without network access only when it passes. For `uncertain` or `failed`, add a short `agent_cache_notes` and fetch only the disputed or missing minimum fragment. The result is conversational and ephemeral; it is not persisted evidence or proof of correctness, current-source freshness, or verbatim wording.
...
A live-source packet that is `verified` and covers a coherent complete target slice may cross the snapshot materialization gate. `load-tech-learning-source` hands it to `study-tech-learning`, the sole snapshot writer; the loader does not persist the packet. Materialization and structural validation occur before teaching that new slice, while learner activity remains a separate state.
...
Omit empty repetition, not the evidence fields. Use short quotations only when wording itself matters. The packet is source input, not proof of study, completion, independent performance, or mastery.
