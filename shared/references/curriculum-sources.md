...
Every registered material source has exactly one machine-readable registry role. `teaching-open` is a public tutorial candidate, `technical-authority` is an official technical source that may be used as the documented fallback teaching path, `physical-book` is a user-confirmed edition and chapter, and `agent-fallback` is bounded material generated by AI when higher-ranked sources are unavailable or unsuitable. Job evidence and priority decisions are not material sources and must not be registered here. A source role does not prove that a particular task section is suitable or that the learner accessed it.

This packaged file defines only the format and selection rules. The Vault owns all actual registrations at `20-学习主线/资料来源注册表.md`; a missing registry is an initialization or migration error, not permission to interpret these examples as data. Register the material sources you actually use by adding one line per source in exactly this format (the indented examples are illustrative and are not registered):
...
- Job evidence and priority decisions remain in job analysis and route decisions; they are never material registry entries.
- `agent-fallback` marks bounded AI-authored material and never proves course coverage.
...
Use these machine-parseable, one-line bindings in `20-学习主线/20-任务包注册表.md`. Keep the field names and pipe order unchanged so every declaration is bound to one stable task-package ID:
...
The task package registry owns task definitions and material-source bindings. Stage files own ordering. The Vault material source registry (`20-学习主线/资料来源注册表.md`) owns registered source identity and locators; this packaged file only defines their format and selection rules.
